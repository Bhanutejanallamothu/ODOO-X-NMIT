...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-[12px] bg-paper-surface text-paper-text border border-paper-border px-2 shadow-[inset_2px_2px_5px_rgba(255,255,255,0.7),8px_8px_18px_rgba(0,0,0,0.05),-6px_-6px_14px_rgba(255,255,255,0.8)] ${className || ''}`}>
          ...
          >
            {cloneElement(icon, {
              className: `w-6 h-6 transition-opacity duration-100 ease-in-out ${
                activeIndex === index ? 'opacity-100 text-brand-600' : 'opacity-40 text-paper-muted'
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-11 h-[5px] rounded-full bg-brand-600 shadow-[0_30px_15px_rgba(79,70,229,0.3)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName || ''}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-30%] top-[5px] w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-brand-600/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
